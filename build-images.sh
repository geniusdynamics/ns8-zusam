...
# Reuse existing nodebuilder-zusam container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-zusam; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-zusam -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/zusam/zusam:0.5.5" \
    "${container}"
# Commit the image
